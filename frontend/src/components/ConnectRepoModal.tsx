...
const WEBHOOK_URL = `${BASE.replace("/api", "")}/webhook`

/* ── step types ──────────────────────────────────────────────────── */

type Phase = "setup" | "connect" | "done"
type Step =
  | "s1" | "s2" | "s3" | "s4" | "s5" | "s6"  // 6 setup slides
  | "input" | "confirm" | "connecting"           // connect phase
  | "done" | "error"                             // result

const SETUP_STEPS: Step[] = ["s1", "s2", "s3", "s4", "s5", "s6"]
...
const SETUP_ORDER: Step[] = ["s1","s2","s3","s4","s5","s6","input","confirm","connecting"]
